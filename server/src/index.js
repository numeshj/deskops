...
// every free host (Render, Koyeb, Fly) terminates TLS at a proxy — without this
// secure cookies never set and req.protocol is always "http"
 ...
 * to run the API alone (local dev uses Vite's own server).
 */
...
async function boot() {
  ...
  app.listen(port, "0.0.0.0", () => {
    ...
    console.log(`  web served from this process: ${SERVE_WEB && fs.existsSync(WEB_DIST) ? "yes" : "no"}`);
  });
}
